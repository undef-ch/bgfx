...
GLuint CompileShaders(const char * vCode, const char * fragCode, const char* geomCode=nullptr){
	...
	if ( InfoLogLength > 0 ){
		glGetShaderInfoLog(VertexShaderID, 4096, &InfoLogLength, log);
		printf("%s\n", log);
	}
	...
	if ( InfoLogLength > 0 ){
		glGetShaderInfoLog(FragmentShaderID, 4096, &InfoLogLength, log);
		printf("%s\n", log);
	}
	...
	if ( InfoLogLength > 0 ){
		glGetShaderInfoLog(ProgramID, 4096, &InfoLogLength, log);
		printf("%s\n", log);
	}
...
static const char* vrImplOpenHMDVert = R"(
									   ...
									   void main(){
										...
										UV = (vertexPosition_modelspace.xy+vec2(1,1))/2.0*vec2(2160, 1200);
									   }
...
static const char* vrImplOpenHMDFrag = R"(
									   ...
									   void main(){
											...
											vec2 LensCenter = UV.x < 1080 ? LeftLensCenter : RightLensCenter;
											vec2 ScreenCenter = UV.x < 1080 ? LeftScreenCenter : RightScreenCenter;

											vec2 oTexCoord = UV.xy / vec2(2160, 1200);
...
static const char* vrImplOpenHMDGeom = R"(
									   ...
									   uniform vec2 Scale = vec2(0.25,0.5);//vec2(0.25,0.5);
									   uniform float DistortionScale = 1.45;//1.45;//1.5;//1.0
...
bool VRImplOpenHMDGL::createSwapChain(const VRDesc& _desc, int _msaaSamples, int _mirrorWidth, int _mirrorHeight)
{
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, m_msaaTexture, 0);
